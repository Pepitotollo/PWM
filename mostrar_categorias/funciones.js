function rellenar_cuadrados () {
    fetch('/PWM/base_datos.json') // Realiza una solicitud HTTP para cargar el archivo JSON
        ...
        .then(data => {
            ...
            for (let j = 0; j < 3; j++) {
                ...
                for (let i = 0; i < 3; i++) {
                    ...
                    let variable = "espacio" + (j+1) + "_div" + (i + 1);
                    console.log(variable);
                    variableValue = window[variable];
                    variableValue.style.backgroundImage = `url(${categorias[i].imagen})`;
                    ...
                    nuevoDiv.textContent = categorias[i].nombre; // Agrega texto al nuevo div
